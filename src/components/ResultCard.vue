...
<template>
  <div class="card overflow-hidden">
    <!-- 卡片头部 -->
    <div class="flex items-center gap-3 border-b border-gray-100 bg-gray-50/60 px-5 py-4 sm:px-6 dark:border-gray-800 dark:bg-gray-800/40">
      <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-500 text-white shadow-sm">
        <Check class="h-4.5 w-4.5" :stroke-width="3" />
      </div>
      ...
      <button
        @click="toggleQr"
        class="flex h-8 shrink-0 items-center gap-1.5 rounded-full bg-gray-100 px-3 text-[11px] font-bold text-gray-600 transition-colors hover:bg-indigo-50 hover:text-indigo-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-indigo-900/30 dark:hover:text-indigo-300"
        title="生成二维码，手机扫码取图"
      >
    ...
    <div class="grid grid-cols-1 gap-5 p-4 sm:p-6 lg:grid-cols-12">
      <!-- 左：预览图 -->
      <div class="flex flex-col items-center justify-center gap-4 lg:col-span-4 lg:border-r lg:border-gray-100 lg:pr-6 dark:lg:border-gray-800">
        <div class="group relative aspect-[4/3] w-full max-w-[280px]">
          <div class="relative h-full overflow-hidden rounded-xl bg-gray-100 ring-1 ring-gray-200/60 transition-transform duration-300 group-hover:scale-[1.02] dark:bg-gray-800 dark:ring-gray-700/60">
            <img
              :src="previewSrc"
            ...
            <div class="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
              <a :href="info.url" target="_blank" class="translate-y-2 transition-all duration-300 group-hover:translate-y-0">
                <div class="flex items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-semibold text-gray-900 shadow-lg transition-transform hover:scale-105">
        ...
        <div v-if="info.thumbnailUrl" class="inline-flex items-center gap-1 rounded-full bg-gray-100 p-1 text-xs font-medium text-gray-500 dark:bg-gray-800 dark:text-gray-400">
          <button
            :class="baseUrl === 'cdn' ? 'bg-white text-indigo-600 shadow-sm dark:bg-gray-700 dark:text-indigo-300' : 'hover:text-gray-700 dark:hover:text-gray-200'"
            class="rounded-full px-3 py-1 transition-all"
            @click="baseUrl = 'cdn'"
          >原图</button>
          <button
            :class="baseUrl === 'thumb' ? 'bg-white text-indigo-600 shadow-sm dark:bg-gray-700 dark:text-indigo-300' : 'hover:text-gray-700 dark:hover:text-gray-200'"
            class="rounded-full px-3 py-1 transition-all"
            @click="baseUrl = 'thumb'"
          >缩略图</button>
        </div>
      ...
      <div class="flex flex-col gap-3 lg:col-span-8">
        <div
          ...
          :key="f.key"
          class="group rounded-xl ring-1 ring-gray-200 transition-all hover:ring-indigo-300 hover:shadow-md dark:ring-gray-800 dark:hover:ring-indigo-700"
        >
          <div class="flex items-center justify-between gap-3 px-4 py-2">
            ...
            <button
              ...
              :class="justCopied === f.key
                ...
                : 'bg-gray-100 text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-indigo-900/30 dark:hover:text-indigo-300'"
            >
        ...
        <button
          ...
          class="flex w-full items-center justify-center gap-1.5 rounded-xl border border-dashed border-gray-200 py-2 text-xs font-semibold text-gray-500 transition-colors hover:border-indigo-300 hover:text-indigo-600 dark:border-gray-700 dark:text-gray-400 dark:hover:border-indigo-700 dark:hover:text-indigo-400"
        >
